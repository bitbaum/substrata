...
export const RESEARCH_PROGRAMMES: readonly ResearchProgramme[] = [
  {
    ...
    question:
      'When a machine starts improving its own hardware, which physical layer sets the pace — and where in the chain does that layer bind?',
    ...
    framing: [
      'Recursive self-improvement is usually discussed as a software event: a model that ' +
        'improves the model. But every improvement that reaches the physical world has to ' +
        'pass through a loop with a physical turn in it — design a part, make it, measure ' +
        'it, design again — and the rate of that loop is set by its slowest edge, not by ' +
        'the intelligence driving it. This programme maps those edges.',
      'The layers below are ordered by the period of one turn, from minutes to years. ' +
        ...
        'rewrite its own code in minutes still waits weeks for a board and years for a ' +
        'qualified material. Where the periods jump is where the chain gates, and the ' +
        'rows of the coverage universe that gate each layer are named against it.',
